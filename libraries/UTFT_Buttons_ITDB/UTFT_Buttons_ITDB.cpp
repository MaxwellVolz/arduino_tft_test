/*
  ...
  of the UTFT and ITDB02_Touch libraries.
...
UTFT_Buttons::UTFT_Buttons(UTFT *ptrUTFT, ITDB02_Touch *ptrITDB02_Touch)
{
	_UTFT = ptrUTFT;
	_ITDB02_Touch = ptrITDB02_Touch;
	deleteAllButtons();
...
{
    if (_ITDB02_Touch->dataAvailable() == true)
    {
		_ITDB02_Touch->read();
		int		result = -1;
		int		touch_x = _ITDB02_Touch->getX();
		int		touch_y = _ITDB02_Touch->getY();
		word	_current_color = _UTFT->getColor();
		...
		while (_ITDB02_Touch->dataAvailable() == true) {};
